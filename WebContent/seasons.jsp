...
<f:view>
	...
	<%
		String host = "10.122.13.10";
			...
			try {
				List friend_list = service.listExistingfriends(user);
		...
		<table width="550px">
			<tr height="75px">
				<td style="font-size: 25px; float: none;color: #0b92d6;">
					<img alt="Seasons Greetings" src="images/Tag.png" width="50px" height="50px" style="float: left;">
					<div style="float: left; margin-top: 10px;">Greetings</div>
				</td>
			</tr>
		</table>	
<%
		String from=request.getParameter("from");
		String festive=request.getParameter("festive");
		session.setAttribute("from",from);
		session.setAttribute("festive",festive);
		if(from!=null)
		{				
			List rs=service.findoldfriends(from,festive);
			int size=rs.size();
			if(size==0)
			{
%>
				<table width="550px">
					<tr>
						<td>
							<img alt="Wishbook" src="images/error.png" width="35px" height="35px" style="float: left; margin-left: 35px">
							<div style="font-size: 15px; color: #0b92d6; float: left; margin-top: 8px">All your friends are already in the wishlist tagged with <%= festive %></div>
						</td>
					</tr>
				</table>
<%
...
%>
			<form action="setGreetingSuccess.jsp">
			<table width="550px" style="color: gray;">
				<tr>
					<td class="topicStyle">
						Select your friends to add them to the wish list tagged with the <%= festive %> festival
					</td>
				</tr>
				<tr style="height: 10px"></tr>
				<tr>
				<td style="float: none;">
								<label style="width: 20px; margin-bottom: 10px; margin-top: 25px; margin-right:8px; margin-left: 30px; color: gray; font-weight: bold; float: left;">To:</label>
								<select id="txtTo" name="txtTo" style="width: 475px; margin-top: 20px; margin-bottom: 10px; border-style: solid;border-color: #cbcacf; border-width: 2px; float: left;">
<%						
			    for(int i=0;i<size;i++)
			    {
    				String s=(String)rs.get(i);
%>	
								<option value="<%=s %>" style="height: 20px; "><%= service.returnName(s) %></option>
<%
				}			
%>	
				</select>
...
<%
		...
		{
%>
			<form action="seasons.jsp">
			<input type="hidden" id="from" name="from" value="<%= user %>">
			<table>				
				<tr>
					<td class="topicStyle" style="width: 251px">
						Select a festival to edit the tagged wishlist
					</td>
					<td style="width: 129px">
						<select name="festive" class="inputFieldStyle">
							<option value="MakarSankranti">MakarSankranti</option>
