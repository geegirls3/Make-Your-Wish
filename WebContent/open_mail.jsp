...
//						String username = "deepakkrishnan";
						...
						String subjectCurr = request.getParameter("subjectCurr");
						Calendar c=Calendar.getInstance();
						String dateTemp = request.getParameter("dateCurr");
						c = CalendarUtility.getCalendarFromString(dateTemp);
						Date dateCurr = c.getTime();
						String subjectSubstringToSearch = subjectCurr;
...
<%
    						...
    						for (int i = 0; i < found.length; i++)
    						{
        						...
        						String fromTemp = "\"" + fromCurr + "\"" + " " + "<" + fromCurr + ">";
        						if(((from[0].toString()).equals(fromCurr) || (from[0].toString()).equals(fromTemp)) && date.equals(dateCurr))
        						{
        							...
        							{        								
%>									 
										<table id="mailContent" style="text-align: left; color: #adadad; width: 550px; max-width: 550px; overflow: hidden;">
<% 
 			      							out.println("<tr><td style=\"width: 50px;\"><b style=\"color: gray;\">From:</b></td><td style=\"width: 500px; text-align: left;\">"+service.returnName(from[0].toString())+"</td></tr><tr style=\"height: 10px;\"></tr>");
        									out.println("<tr><td style=\"width: 50px;\"><b style=\"color: gray;\">Subject:</b></td><td style=\"width: 500px; text-align: left;\">"+subjectCurr+"</td></tr><tr style=\"height: 10px;\"></tr>");
        									out.println("<tr><td colspan=\"2\" style=\"border-style: solid;border-color: #cbcacf; border-width: 2px; width: 550px; max-width: 550px; overflow: hidden; padding: 10px 10px 10px 10px;\">"+(String)o+"</td></tr>");
%>
											<tr height="10px"></tr>
											<tr><td>
												<a href="replyMail.jsp?name=<%= service.returnName(from[0].toString()) %>&to=<%= from[0].toString() %>&subject=<%= m.getSubject().toString() %>">											
													<img id="menuBottomFAQLinkImage" src="images/reply.png" height="25px" width="25px" title="Reply"/>
												</a>												
											</td></tr>
										</table>
...
//										out.println(path);
							...
							/*						for (int k=0; file.exists(); k++) {
		  												file = new File(path+part.getFileName()+k);
													}*/
    												mbp.saveFile(file);
    												out.println("File saved to desktop");
  												}
  												if (disposition == null) 
  												{
  													// Check if plain
