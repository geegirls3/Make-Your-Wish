...
{
Calendar c = CalendarUtilityDefault.getCalendarFromString(anniversary);
ann = c.getTime();
s.setAnniversary(ann);
}
...
s.setUname(uname);
s.setTw(request.getParameter("tw"));
s.setFb(request.getParameter("fb"));
s.setPe(request.getParameter("pe"));
s.setLi(request.getParameter("li"));
s.setPropic(request.getParameter("propic"));


int j=ws.adduser_main(s,0);
//out.println("successfully updated and user id is "+j);
...
%>
								...
								<div style="font-size: 15px; color: #0b92d6; float: left; margin-top: 8px">Profile has been updated</div>
							</td></tr>						
							</table>
				</div>
						...
						<td width="200px" style="overflow: auto; text-align: left; color: #a1a0a8; padding-left: 5px; font-family: sans-serif; font-size: 12px;">
							<%								
								for(int i=0;i<size1;i++){
	    							user_info s1=(user_info)currUserInfo.get(i);
	    							out.println("<b style=\"font-size:15px;\">" + s1.getFirstName()+ " " + s1.getLastName() + "</b><br/>");
	    							Calendar c=Calendar.getInstance();
									c.setTime(s1.getDob());
									String dob1=CalendarUtilityDefault.getStringFromCalendar(c);
	    							out.println(dob1 + "<br/>");
	    							out.println(s1.getCity() + "<br/>");
	    							out.println(s1.getCountry() + "<br/>");
	    							out.println(s1.getPhoneNo() + "<br/>");
	    							out.println(s1.getEmail() + "<br/>");
	    						}	    						
							%>
						</td>
					</tr>
